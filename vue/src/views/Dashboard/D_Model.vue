    ...
    <div class="app-embed-container">
        ...
        <div class="model-decision-content">
            ...
            <div v-if="currentView === 'bubbles'" class="bubble-visualization">
                ...
                <!-- 桌面/平板视图 (气泡图) -->
                <div v-if="!isMobile" class="bubble-field-wrapper">
                    <div class="bubble-field-content">
...
export default {
    ...
    data() {
        ...
        const domainsData = [
            // 中心大模型 (不可点击切换)
            { name: '大模型', en: 'LLM Core', size: 220, color: '#1B6AA5', class: 'center-bubble' },
            // 周围七个子领域 (点击后切换到聊天模式)
            { name: '采煤', en: 'Coal Mining', size: 140, color: '#909399', class: 'pos-1' , duration: generateRandomDuration(), delay: generateRandomDelay(), amplitudeClass: generateRandomAmplitudeClass() },
            { name: '掘进', en: 'Roadway Excavation', size: 140, color: '#F56C6C', class: 'pos-2' , duration: generateRandomDuration(), delay: generateRandomDelay(), amplitudeClass: generateRandomAmplitudeClass() },
            { name: '安装回撤', en: 'Equipment Installation & Retreat', size: 140, color: '#E6A23C', class: 'pos-3' , duration: generateRandomDuration(), delay: generateRandomDelay(), amplitudeClass: generateRandomAmplitudeClass() },
            ...
            { name: '机电', en: 'Mechanical & Electrical', size: 140, color: '#67C23A', class: 'pos-5', duration: generateRandomDuration(), delay: generateRandomDelay(), amplitudeClass: generateRandomAmplitudeClass() },
            { name: '通风', en: 'Ventilation', size: 140, color: '#6F42C1', class: 'pos-6' , duration: generateRandomDuration(), delay: generateRandomDelay(), amplitudeClass: generateRandomAmplitudeClass() },
    ...
    methods: {
        ...
        selectDomain(domainName) {
            if (domainName !== '大模型') {
                ...
                this.$message.success(`已进入 ${domainName} 智能决策模式。`);
...
/* 气泡场内部内容 */
.bubble-field-content {
    position: absolute; /* 绝对定位到 wrapper 内部 */
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
...
/* 呼吸动画：必须在动画中包含 translate(-50%, -50%) 偏移，以防止动画覆盖定位 */
@keyframes pulse-sm {
...
/* 悬停效果：取消呼吸动画，应用更明显的放大效果，同时保留居中偏移 */
...
/* 7个小气泡的响应式定位 (使用百分比) */
/* --------------------------------------------------- */
/* 气泡中心点相对于 bubble-field-content 的 top/left */
.center-bubble { top: 36%; left: 50%; font-size: 24px; z-index: 15; cursor: default; }

.center-bubble:hover { 
    transform: translate(-50%, -50%); /* 保持基准偏移，不缩放 */
}

.pos-1 { top: 43%; left: 19%; } /* 采煤 */
.pos-2 { top: 60%; left: 37%; } /* 掘进 */
.pos-3 { top: 60%; left: 66%; } /* 安装回撤 */
.pos-4 { top: 43%; left: 81%; } /* 辅助运输 */
.pos-5 { top: 21%; left: 76%; } /* 机电 */
.pos-6 { top: 10%; left: 50%; } /* 通风 */
.pos-7 { top: 21%; left: 23%; } /* 应急救援 */
/* --------------------------------------------------- */
