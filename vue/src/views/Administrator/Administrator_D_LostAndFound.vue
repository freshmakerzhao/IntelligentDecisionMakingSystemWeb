                ...
                <template slot-scope="props">
                    <el-form label-position="left" inline class="demo-table-expand">
                        <el-form-item label="物品类别：">
                            <span>{{ props.row.typeName ? props.row.typeName : "/"}}</span>
                        </el-form-item>
                        <el-form-item label="价值：">
                            <span>{{ props.row.value }}</span>
                        </el-form-item>
                        <el-form-item label="悬赏：">
                            <span>{{ props.row.reward ? props.row.reward : "/"}}</span>
                        </el-form-item>
                    </el-form>
                </template>
            </el-table-column>
...
<script>
    export default {
        ...
        methods:{
            // 从后台获取数据，并在页面中赋值
            async load() {
                await this.request.get("/lostAndFound/page", {
            ...
            // 失效触发
            del(id) {
                this.request.delete("/lostAndFound/del/" + id).then(res => {
                    if (res.code === "200") {
