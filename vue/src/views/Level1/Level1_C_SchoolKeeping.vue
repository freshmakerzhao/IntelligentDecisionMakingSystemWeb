<template>
    <div>
        <div style="margin: 10px 0">
            ...
            <el-input style="width: 200px" class="ml-5" placeholder="请输入描述关键字" suffix-icon="el-icon-search" v-model="taskContent"></el-input>
                ...
                <template slot-scope="props">
                    <el-form label-position="left" inline class="demo-table-expand">
                        <el-form-item label="完成时间：">
                            <span>{{ formatDateV2(props.row.taskTime) }}</span>
                        </el-form-item>
                    </el-form>
                </template>
            </el-table-column>
            ...
            <el-table-column label="任务描述">
                <template slot-scope="scope">
                    <el-popover trigger="hover" placement="top" width="300">
                        ...
                        <div slot="reference">
                            <span class="ellipsis">{{scope.row.taskContent}}</span>
                        </div>
                    </el-popover>
                </template>
            </el-table-column>
...
<script>
    export default {
        ...
        methods:{
            // 从后台获取数据，并在页面中赋值
            async load() {
                await this.request.get("/schoolKeeping/page", {
            ...
            // 失效触发
            del(id) {
                this.request.delete("/schoolKeeping/del/" + id).then(res => {
                    if (res.code === "200") {
