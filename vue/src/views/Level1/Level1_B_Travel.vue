<template>
    <div>
        <div style="margin: 10px 0">
            ...
            <el-input style="width: 200px" class="ml-5" placeholder="请输入出发地" suffix-icon="el-icon-search" v-model="fromAddress"></el-input>
            <el-input style="width: 200px" class="ml-5"  placeholder="请输入目标地" suffix-icon="el-icon-search" v-model="toAddress"></el-input>
                ...
                <template slot-scope="props">
                    <el-form label-position="left" inline class="demo-table-expand">
                        <el-form-item label="出发地：">
                            <span>{{ props.row.fromAddress ? props.row.fromAddress : "/"}}</span>
                        </el-form-item>
                        <el-form-item label="出发时间：">
                            <span>{{ formatDateV2(props.row.fromTime) }}</span>
                        </el-form-item>
                        <el-form-item label="目标地：">
                            <span>{{ props.row.toAddress ? props.row.toAddress : "/"}}</span>
                        </el-form-item>
                        <el-form-item label="抵达时间：">
                            <span>{{ formatDateV2(props.row.toTime) }}</span>
                        </el-form-item>
                        <el-form-item label="空闲位置：">
                            <span>{{ props.row.freePosition ? props.row.freePosition : "/"}}</span>
                        </el-form-item>
                    </el-form>
                </template>
            </el-table-column>
...
<script>
    export default {
        ...
        methods:{
            // 从后台获取数据，并在页面中赋值
            async load() {
                await this.request.get("/travel/page", {
            ...
            // 失效触发
            del(id) {
                this.request.delete("/travel/del/" + id).then(res => {
                    if (res.code === "200") {
